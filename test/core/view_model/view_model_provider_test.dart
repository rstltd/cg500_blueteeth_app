...
void main() {
  ...
  group('ViewModelSelector', () {
    testWidgets('should rebuild when selected value changes', (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: ViewModelProvider<_MultiPropertyViewModel>(
            create: () => _MultiPropertyViewModel(),
            builder: (context, viewModel, child) {
              return Column(
                children: [
                  ViewModelSelector<_MultiPropertyViewModel, int>(
                  ...
                  ElevatedButton(
                    ...
                    onPressed: viewModel.changeName,
      ...
      // Increment count (should rebuild selector)
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: ViewModelProvider<_CounterViewModel>(
            create: () => _CounterViewModel(),
            builder: (context, viewModel, child) {
              return ViewModelConsumer<_CounterViewModel>(
  ...
  group('ViewModelListener', () {
    testWidgets('should call listener on changes', (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: ViewModelProvider<_CounterViewModel>(
            create: () => _CounterViewModel(),
            builder: (context, viewModel, child) {
              return ViewModelListener<_CounterViewModel>(
                listener: (context, vm) {
                  notifications.add(vm.count);
                },
                child: Column(
                  children: [
                    Text('Count: ${viewModel.count}'),
      ...
      expect(notifications, contains(1));
    });

    testWidgets('should respect listenWhen condition', (tester) async {
      final notifications = <int>[];
      ...
      // Increment to 1 (odd - should not notify)
      await tester.tap(find.text('Increment'));
      await tester.pump();

      // Increment to 2 (even - should notify)
      await tester.tap(find.text('Increment'));
      await tester.pump();

      // Increment to 3 (odd - should not notify)
      await tester.tap(find.text('Increment'));
      await tester.pump();
...
class _MultiPropertyViewModel extends BaseViewModel {
  int _count = 0;
